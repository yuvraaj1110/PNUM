...
scripts/
  0000_base_schema.sql       # base DDL: profiles, customers, jobs, create_customer RPC, auth trigger
  setup-supabase.sql         # RLS, Realtime, demo seed data (migration 2)
  0002_agentic_upgrade.sql   # agentic + analytics schema (migration 3)
  0003_seed_skills.sql       # skills seed (migration 4)
  0004_grant_views.sql       # grant SELECT on views (migration 5)
  handle_new_user.sql        # legacy reference; trigger now in 0000_base_schema.sql
...
middleware.ts                # Supabase auth session refresh + protected routes
next.config.ts, tailwind.config.ts, postcss.config.mjs, tsconfig.json, eslint.config.mjs
```
...
Postgres (Supabase). Sources: `scripts/0000_base_schema.sql`,
`scripts/setup-supabase.sql`, `scripts/0002_agentic_upgrade.sql`,
`scripts/0003_seed_skills.sql`, `scripts/0004_grant_views.sql`.
...
> created by the committed SQL migrations; skill matching uses the `skills` /
> `technician_skills` tables, not `skills_tags`.

### `customers`

RPC: `create_customer(p_name text, p_address text) → uuid` inserts a customer
and returns its id. `/api/jobs` calls this RPC. Defined in
...
Defined in `scripts/0000_base_schema.sql` — `SECURITY DEFINER` function that
fires `AFTER INSERT ON auth.users FOR EACH ROW`. It inserts a `profiles` row with
`id = new.id`, `full_name = COALESCE(raw_user_meta_data->>'full_name','New
Dispatcher')`, `role = 'Dispatcher'`, and `created_at`/`updated_at = now()`.
The trigger `on_auth_user_created` is created in the same migration.
...
5. `logJobEvent(sb, { jobId, actor: 'dispatcher', eventType: 'created', payload: { title, priority, customer_name } })` — see **Audit API** below.
6. If `assignMode==='now' && selectedPlumberId`: set that plumber's `status` to
   `busy`. If that update fails → **207** `{ error, jobId }`. On success:
   `logJobEvent(sb, { jobId, actor: 'dispatcher', eventType: 'assigned', payload: { technician_id } })`.
...
5. `logJobEvent(sb, { jobId: id, actor: 'dispatcher', eventType, payload: { from: current.status, to: status } })` where `eventType` is `completed`, `cancelled`, or `status_changed`.

**Responses:** **200** `{ success: true, id, status }` · **400** · **404** · **500**.

### Audit API — `logJobEvent` (`lib/audit.ts`)

All routes that mutate jobs call this helper. Signature (object param — **not** positional):
...
logJobEvent(
  ...
  params: {
    jobId: string;
    eventType: 'created' | 'assigned' | 'status_changed' | 'completed' | 'cancelled';
...
Inserts into `job_events`: `{ job_id, actor, event_type, payload }`. Never throws.
