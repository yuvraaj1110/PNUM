...
| # | File | What it does |
|---|---|---|
| 0 | `scripts/0000_base_schema.sql` | **Base DDL** on a fresh project: `profiles`, `customers`, `jobs` tables; `create_customer` RPC; `handle_new_user` auth trigger; demo customer + seed rows for later migrations. |
| 1 | `scripts/setup-supabase.sql` | RLS + permissive demo policies, Realtime on `profiles`/`jobs`, converts `current_location` to `jsonb` if needed, adds columns, seeds plumber profiles + demo jobs. |
...
> **Note — legacy `handle_new_user.sql`:** the auth trigger is now defined in
> `0000_base_schema.sql`. The standalone `scripts/handle_new_user.sql` file is
> kept for reference only; you do not need to run it separately if you applied
> migration 0000.

> **Realtime on Supabase — do not drop the publication.** When enabling Realtime,
> use `ALTER PUBLICATION supabase_realtime ADD TABLE public.<table>` inside a
> `DO $$ … EXCEPTION WHEN duplicate_object THEN NULL` block (as in
> `setup-supabase.sql`). **Never** `DROP PUBLICATION supabase_realtime` — that
> breaks Supabase's managed Realtime on a live project. Use the `pgcrypto`
> extension in `0000_base_schema.sql` (not `postgis`).
