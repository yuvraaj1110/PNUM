...
export default function MiniMapWidget() {
    ...
    return (
        <div className="relative group">
            {/* Map container */}
            <div className="relative h-[560px] rounded-[2rem] overflow-hidden border border-gray-800/50 bg-[#161616]">
                <FleetMap plumbers={plumbers} jobs={jobs} mini />

                {/* Gradient overlay at top */}
                <div className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-[#161616] to-transparent z-10 pointer-events-none" />
                ...
                {/* Bottom gradient */}
                <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#161616] to-transparent z-10 pointer-events-none" />
            </div>

            {/* View Full Map link */}
            <Link
                href="/fleet"
                className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-yellow-400/90 backdrop-blur-sm text-black text-xs font-bold hover:bg-yellow-300 transition-all shadow-xl shadow-yellow-500/20 group/btn"
            >
                Open Fleet Map
                <ArrowUpRight
                    size={14}
