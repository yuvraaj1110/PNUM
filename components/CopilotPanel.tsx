...
export default function CopilotPanel() {
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-[9998] flex items-center gap-2 px-5 py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold shadow-2xl shadow-blue-600/30 transition-all active:scale-95"
      >
        <Sparkles size={18} />
        Dispatch Copilot
      </button>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9998] w-[400px] max-w-[calc(100vw-2rem)] h-[600px] max-h-[calc(100vh-3rem)] flex flex-col bg-[#13151c] border border-gray-800/80 rounded-[1.75rem] shadow-2xl shadow-black/60 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800/60 bg-[#161922]">
            ...
            <div className="space-y-2">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="block w-full text-left px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/5 text-gray-300 text-xs hover:bg-white/5 hover:border-blue-500/20 transition-all"
                >
                  {s}
                </button>
              ))}
        ...
        {display.map((m, i) => (
          <div key={i} className={m.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
            <div
              className={
                m.role === 'user'
                  ? 'max-w-[85%] px-4 py-2.5 rounded-2xl rounded-br-md bg-blue-600 text-white text-sm whitespace-pre-wrap'
                  : m.role === 'system'
                    ? 'max-w-[90%] px-3 py-2 rounded-xl bg-white/[0.03] border border-white/5 text-gray-400 text-xs'
                    : 'max-w-[85%] px-4 py-2.5 rounded-2xl rounded-bl-md bg-[#1f232d] text-gray-200 text-sm whitespace-pre-wrap'
              }
            >
              {m.text}
            </div>
          ...
          <div className="px-4 py-4 rounded-2xl bg-orange-500/5 border border-orange-500/20">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-[#1f232d] hover:bg-[#2a2f3a] text-gray-300 text-xs font-bold border border-gray-800 transition-all"
              >
                <Ban size={14} /> Decline
              </button>
            </div>
      ...
      <form
        ...
        className="p-3 border-t border-gray-800/60 bg-[#161922]"
      >
        <div className="flex items-center gap-2">
          <input
            ...
            disabled={loading}
            className="flex-1 bg-[#1f232d] border border-gray-800 focus:border-blue-500/50 outline-none rounded-xl py-2.5 px-4 text-sm text-white placeholder:text-gray-600 transition-colors disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={loading || !input.trim()}
            className="p-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-40 transition-all"
          >
            <Send size={18} />
          </button>
        </div>
